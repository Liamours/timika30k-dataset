...
def main() -> None:
    ...
    fieldnames = list(source_rows[0].keys()) + ["clahe_relative_path"]
    ...
    out_rows: list[dict] = []
    for r in tqdm(pending, desc="clahe"):
        gray = np.array(Image.open(SOURCE_ROOT / r["preprocessed_relative_path"]))
        ...
        out_rows.append({**r, "clahe_relative_path": str(rel)})
    ...
    if out_rows:
        append_rows(out_rows, fieldnames)
    log.info(f"clahe complete: {len(pending)} written this run, {len(skip) + len(pending)} total")
