...
`E:\dataset\timika-50k\README.md`/`manifest.yaml`, and
...
**How many instances?** 51,659 images across 7 sources (Shenzhen 662,
Montgomery 138, TBX11K 11,490, ChestX-Det 3,577, COVID-19 Radiography
Database 21,165, BIMCV-CAAXR 2,539, SIIM-ACR Pneumothorax 12,088).
...
from their own canonical copies, only excluded from timika-50k's `data/`.
...
- `labels/organ_region/`: 51,659/51,659 images, one 6-zone lung-region
  map each, entirely model-derived (PSPNet + fixed geometry, see
  Preprocessing below), never real ground truth for any image. 8
  `confidence: failed` (valid image, model didn't segment it), 1
  `confidence: invalid` (not a real diagnostic image, see quality_flags).
- `labels/disease/`: 120,634 label files across 7 finding classes for a
  ...
  label**: roughly 19,500 images (TBX11K `sick`, 3 of COVID-19
  Radiography Database's 4 classes, some BIMCV-CAAXR images, both
  sources' withheld competition test splits) have none at all, blocked on
  a disease segmentation model that does not yet work
  (`repo/timika50k_pseudolabels/README.md`, "Current status").
...
instance UIDs, not patient identifiers); `view_position` is `"-"` for 5
of 7 sources with no real per-image or per-source projection info
...
its overlap partners are in timika-50k).
...
2026-09-03 embedding-based QC pass (`src/timika50k_dataset/anomaly/`)
...
1. Per-source canonicalization (grayscale/channel/bit-depth
   ...
   `C:\dataset\{name}\`/`E:\dataset\{name}\`, before timika-50k assembly.
2. `labels/organ_region/`: PSPNet (torchxrayvision `chestx_det.PSPNet`,
   ...
   fixed 6-zone geometry (`analyses/timika50k_organ_labels/`).
3. `labels/disease/`: real masks kept as-is for 3 sources; SAM ViT-B
   ...
   negatives for whole-image-normal reads (`repo/timika50k_pseudolabels/`).
4. `preprocessed/`: one materialized preprocessing variant so far (of 5
   ...
   pipeline itself uses (`analyses/timika50k_preprocessing/`).
...
`repo/timika50k_dataset/` per this repo's own scope.
...
`E:\dataset\timika-50k\README.md`'s "What each source's original labels
...
120,634) is a real box with no verified finding class, not a
...
`no_redistribution: true` at the derived-dataset level (4 of 7 sources
...
(`E:\dataset\timika-50k\`) and a project-local copy
(`dataset/timika-50k/`).

**Licensing.** No single license; inherits each of its 7 sources' own
terms (`dataset/manifest.md`'s timika-50k row). One real, deliberately
...
yet; `timika-50k`'s own composition has changed twice already (37,033 to
51,659 images, 2 source folds-in), each change logged in
`E:\dataset\timika-50k\manifest.yaml`'s own notes rather than tagged as a
separate release.
